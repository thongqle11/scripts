...
#    Modified by Edward Ng for E4
#    Edward.Ng@qlogic.com 
#'''
...
#                  '0x1933': {'modelname': 'Bayview2', 'mf_mode': 'FFA'},
                  '0x00cf': {'modelname': 'HP Quest', 'mf_mode': 'N/A'},
                  '0xe4f1': {'modelname': 'Brussels 40G', 'mf_mode': 'N/A'},
                  '0xe4f7': {'modelname': 'Bavaria-Q', 'mf_mode': 'N/A'},
                  '0xe4f5': {'modelname': 'Bavaria-D', 'mf_mode': 'N/A'},
                  '0x0223': {'modelname': 'HP Quagmire', 'mf_mode': 'N/A'},
                  '0x000d': {'modelname': 'AH: Austin-C', 'mf_mode': 'N/A'},
                  '0x021b': {'modelname': 'AH: HP Queenie', 'mf_mode': 'N/A'},
                  '0x021d': {'modelname': 'AH: HP Quadrangle', 'mf_mode': 'N/A'},
                  '0x021f': {'modelname': 'AH: HP Quick', 'mf_mode': 'N/A'},
                  '0x0001': {'modelname': 'AH: HP Quantity', 'mf_mode': 'N/A'},
                  '0x021e': {'modelname': 'AH: HP Quentina', 'mf_mode': 'N/A'},
                  '0x000b': {'modelname': 'AH: Dell Dunkirk', 'mf_mode': 'N/A'},
		  '0x0006': {'modelname': 'AH: Dell Dresden', 'mf_mode': 'N/A'},
		  '0x0009': {'modelname': 'AH: Dell Dunedin', 'mf_mode': 'N/A'},
		  '0x0026': {'modelname': 'AH: Cavium Bonito 4x10G SFP', 'mf_mode': 'N/A'},
		  '0x0029': {'modelname': 'AH: Cavium Bonito 4x10G RJ45', 'mf_mode': 'N/A'},

                  '0xe4f8': {'modelname': 'BB: Baffin Bay 100G', 'mf_mode': 'N/A'},
                  '0x0074': {'modelname': 'BB: IBM Jawa', 'mf_mode': 'N/A'},
                  '0x0075': {'modelname': 'BB: IBM Jawa - SF', 'mf_mode': 'N/A'},
                  '0x0032': {'modelname': 'BB: IBM Jawa - MF(NIC)', 'mf_mode': 'N/A'},
                  '0x0034': {'modelname': 'BB: IBM Jawa - CNA', 'mf_mode': 'N/A'},
		}
...
def dmidecode(request_type):
    ...
    if request_type == 'bios_version':
        try:
            return re.search('Version: (.*)', os.popen('dmidecode -t bios').read()).group(1)
        except:
            return 'N/A'
...
def get_board_info(driver_name='qede'):
...
def get_sysinfo():
    ...
    #sys_info['OS2'] = run_command('lsb_release -a | grep -i Description | xargs echo -n')[1] 
    sys_info['SP'] = run_command("lsb_release -a | grep -i Description | cut -d ':' -f2- | xargs echo -n")[1] 
    #sys_info['OS2'] = run_command("lsb_release -a | grep -i Description | awk `{$1=""; print $0}' | xargs echo -n")[1] 
    sys_info['Architecture'] = platform.architecture()[0]
    sys_info['SSH IP'] = run_command("ip addr | grep 172 | awk '{print $2}' | awk -F'/' '{print $1}' | xargs echo -n")[1]
    ...
    sys_info['qed'] = get_driver_version('qed')
    sys_info['qede'] = get_driver_version('qede')
    sys_info['qedi'] = get_driver_version('qedi')
    sys_info['qedf'] = get_driver_version('qedf')
    sys_info['qedr'] = get_driver_version('qedr')
    ...
    sys_info['BIOS Version'] = dmidecode('bios_version')
