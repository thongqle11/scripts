...
do
	...
	sleep $TIMEOUT
	echo "$(date +"%D %T")  Issuing target reset to device $DEVICEID1"
	logger "**** Issuing target reset to device $DEVICEID1 ****"
	sg_reset -t $DEVICEID1 > /dev/null
	sleep $TIMEOUT
	...
	sleep $TIMEOUT
	echo "$(date +"%D %T")  Issuing target reset to device $DEVICEID2"
	logger "**** Issuing target reset to device $DEVICEID2 ****"
	sg_reset -t $DEVICEID2 > /dev/null
	sleep $TIMEOUT
done
