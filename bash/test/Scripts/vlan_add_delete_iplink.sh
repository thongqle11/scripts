...
clear
    ...
    while true; do
        ...
        echo "$(date) : ip link add link $int1 name $int1.$vlan type vlan id $vlan"
        #vconfig add $int1 $vlan &>/dev/null
	ip link add link $int1 name $int1.$vlan type vlan id $vlan
	sleep 3
	...
	echo "$(date) : ip link delete $int1.$vlan"
        #vconfig rem $int1.$vlan &>/dev/null
	ip link delete $int1.$vlan
        sleep 10
