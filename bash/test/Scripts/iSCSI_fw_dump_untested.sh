...
do
	...
	echo "$(date +"%D %T")  Performing FW Dump on /sys/class/iscsi_host/$DEVICEID1"
	logger "Performing FW Dump on /sys/class/iscsi_host/$DEVICEID1"
        echo 3 > /sys/class/iscsi_host/$DEVICEID1/device/fw_dump
	sleep $TIMEOUT
	...
	echo "$(date +"%D %T")  Performing FW Dump on /sys/class/iscsi_host/$DEVICEID2"
	logger "Performing FW Dump on /sys/class/iscsi_host/$DEVICEID2"
        echo 3 > /sys/class/iscsi_host/$DEVICEID2/device/fw_dump
	sleep $TIMEOUT
