...
/usr/local/src/netperf-2.6.0/src/super_netperf.sh  64 -H 192.168.1.166 -t TCP_SENDFILE -l $3 >  TxRunRes0.txt &
if [ $Dual -eq 1 ]; then 
/usr/local/src/netperf-2.6.0/src/super_netperf.sh  64 -H 192.168.2.166 -t TCP_SENDFILE -l $3 >  TxRunRes1.txt &
fi;
fi
if [ $Rx == 1 ]; then 
ssh $RemoteIP "/usr/local/src/netperf-2.6.0/src/super_netperf.sh  64 -H 192.168.1.164 -t TCP_SENDFILE -l $3" >  RxRunRes0.txt & 
if [ $Dual -eq 1 ]; then 
ssh $RemoteIP "/usr/local/src/netperf-2.6.0/src/super_netperf.sh  64 -H 192.168.2.164 -t TCP_SENDFILE -l $3" >  RxRunRes1.txt & 
fi
fi 
...
if [ $PingCount -eq 0 ]; then
  # 100% failed
echo "Conetivity test port 0 faild after test!!"
fi
PingCount=$(ping -c 1 192.168.2.166 | grep 'received' | awk -F',' '{ print $2 }' | awk '{ print $1 }')
if [ $PingCount -eq 0 ]; then
  # 100% failed
echo "Conetivity test port 1 faild after test!!"
fi
...
#Idle dump
echo dump > /sys/kernel/debug/qed/21:00:01/idle_chk
cat /sys/kernel/debug/qed/21:00:01/idle_chk > idle_chk.txt
echo Done! 
